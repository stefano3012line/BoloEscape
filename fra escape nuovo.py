...
while running:
    ...
    if int(counter) == Bonati_spawn_value:
        Bonati_spawn_value = counter + np.random.randint(7,13) 
        Bonati.hp = 1
        print('Bonati',counter, Bonati_spawn_value)
    if Bonati.hp == 1:
    ...
    #####################################################################################################################
    #aggiungo Lamanna
    #if int counter è solo un proof of concept poi tocca fare una cosa seria per ora bonati spawna quando lo counter è divisibile per 17 e despowna quando viene colpito
    if int(counter) == lamanna_spawn_value:
        ...
        lamanna_spawn_value = counter + np.random.randint(10,17)
        Lamanna.hp = 1
    if Lamanna.hp == 0:
...
font = game.font.SysFont('Monocraft', 70)
counter_text = font.render(f"counter: {int(counter)}", True, text_color)
screen.blit(counter_text, (300, 480))
